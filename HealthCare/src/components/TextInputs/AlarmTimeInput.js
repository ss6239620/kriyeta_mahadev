...
export default function AlarmTimeInput({ inputTitle, isRequire, style, isiconRequire, textInputParams, handleChange }) {
    ...
    function onChange(event, selectedDate) {
        ...
        const fDate = tempDate.getMinutes() - currTime.getMinutes()
        setText(fDate);
        ...
        <View>
            <View style={{ ...style }}>
                ...
                <TouchableOpacity style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderColor: colorTheme.borderColor, borderBottomWidth: 1 }} onPress={() => setshow(true)}>
                    <Text style={{ marginLeft: 5, color: 'gray', fontSize: 15, fontWeight: '400' }}>{text === '' ? 'Enter Time' : `${text} min` }</Text>
